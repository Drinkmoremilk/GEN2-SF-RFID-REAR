...
% B = 2;                                                % bandwidth(MHz)
% N = 200;                                               % Number of subcarriers (IFFT points), N < (B/fBLF*mu), mu = 1/2 when meeting 1/0 bit, respectively.
...
% TS_t = round(ifft(ifftshift(TS_f)), 4).*win;     % Training sequence in the time domain
